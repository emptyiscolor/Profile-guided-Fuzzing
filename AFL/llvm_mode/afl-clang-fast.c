...
if (getenv("AFL_VAR_REC")) {
  u8 found = 0;
  char *obj_str = getenv("AFL_VAR_REC");
  for (int i = 0; i < cc_par_cnt ; i++)
  {
    if(strcmp(obj_str, cc_params[i]) == 0)
    {
      cc_params[cc_par_cnt++] = alloc_printf("%s/var_mode/var.o", obj_path);
      found = 1;
      break;
    }
  } 
  if (!found) cc_params[cc_par_cnt++] = alloc_printf("%s/var_mode/var_empty.o", obj_path);
}
#endif
