...
/* Setup shared map for fuzzing with virtual FS*/
void setup_vfs_shmem(afl_state_t *afl) {

  afl_forkserver_t* fsrv = &afl->fsrv;
  /* Do not use 0x13337 as FS_AFL_SHM_ID, which is reserved for FS testing*/
  u8 *fs_shm_key_str = getenv("FS_AFL_SHM_ID");
  fsrv->fs_shm_key = fs_shm_key_str ? atoi(fs_shm_key_str) : 13337;
  u32 key_fs = ftok("/tmp/", fsrv->fs_shm_key);
  if (key_fs == -1)
    PFATAL("ftok error");

  fsrv->fs_afl_shm_id = shmget(key_fs, MAX_FSMAP, IPC_CREAT | 0600);

  if (fsrv->fs_afl_shm_id < 0)
    FATAL("shmget() failed");

  fsrv->fs_shared_memory = shmat(fsrv->fs_afl_shm_id, NULL, 0);

  fsrv->abs_fp = (abs_file_t *)fsrv->fs_shared_memory;

  if (fsrv->out_file) {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wstringop-truncation"
    strncpy(fsrv->abs_fp->dfile.filename, fsrv->out_file, FN_LEN);
