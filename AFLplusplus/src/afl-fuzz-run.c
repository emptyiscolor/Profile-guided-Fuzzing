...
static void write_with_gap(afl_state_t *afl, u8 *mem, u32 len, u32 skip_at,
  ...
  */
  ...
  if (afl->fsrv.fs_shm_key) {
    struct stat st;

    if (!fs_buffer && fstat(fd, &st) != 0)
      FATAL("Failed to stat");

    if (!fs_buffer) memcpy(&afl->fsrv.abs_fp->dfile.stat, &st, sizeof(st));
    else afl->fsrv.abs_fp->dfile.stat.st_size = len;

    fs_buffer = afl->fsrv.abs_fp->dfile.contents;

    if (fs_buffer == (void *)-1)
      FATAL("[fs_buffer] shmat() failed");

    if (!post_process_skipped) {
      if (new_size && new_size > MIN_FSMAP)
        FATAL("FILE TOO LARGE");

      afl->fsrv.abs_fp->dfile.stat.st_size = new_size;
      memcpy(fs_buffer, new_mem, new_size);
    } else {
      if (skip_at + skip_len > MIN_FSMAP)
        FATAL("FILE TOO LARGE");

      if (skip_at) {
        afl->fsrv.abs_fp->dfile.stat.st_size = skip_at;
        memcpy(fs_buffer, mem, skip_at);
        afl->fsrv.abs_fp->offset = skip_at;
      }

      if (tail_len) {
        afl->fsrv.abs_fp->dfile.stat.st_size = afl->fsrv.abs_fp->offset + tail_len;
        memcpy(fs_buffer +  afl->fsrv.abs_fp->offset, mem + skip_at + skip_len, tail_len);
      }
    }

    afl->fsrv.abs_fp->offset = 0;
    afl->fsrv.abs_fp->fd = SHM_AFL_FD;
    afl->fsrv.abs_fp->info.flock_release = 0;
  }
