...
static u8   debug;
static u8   cwd[4096];
static u8   cmplog_mode, var_rec_mode;
u8          use_stdin;                                             /* dummy */
...
static void edit_params(u32 argc, char **argv, char **envp) {
  ...
  if (compiler_mode == LLVM || compiler_mode == LTO) {
    ...
    if (var_rec_mode) {
        ...
        cc_params[cc_par_cnt++] = "-fexperimental-new-pass-manager";
        cc_params[cc_par_cnt++] =
            alloc_printf("-fpass-plugin=%s/afl-llvm-var-rec.so", obj_path);
      #else

        cc_params[cc_par_cnt++] = "-Xclang";
        cc_params[cc_par_cnt++] = "-load";
        cc_params[cc_par_cnt++] = "-Xclang";
        cc_params[cc_par_cnt++] = alloc_printf("%s/afl-llvm-var-rec.so", obj_path);
      #endif
    }
...
#undef NATIVE_MSG
  ...
  var_rec_mode = getenv("AFL_VAR_REC") || getenv("AFL_LLVM_VAR_REC");

  if (!be_quiet && var_rec_mode)
    printf("VarRec mode by binpang\n");
