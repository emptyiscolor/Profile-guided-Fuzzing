...
#if defined PROFILING_FORK || PROFILING_NYX || PROFILING_SYS_USR || PROFILING
...
void afl_fsrv_start(afl_forkserver_t *fsrv, char **argv,
  ...
  if (!fsrv->fsrv_pid) {
    ...
    snprintf(dev_null_buffer, 10, "%d", fsrv->dev_null_fd);
  ...
  if (likely(fsrv->use_shmem_fuzz)) {
    ...
    *fsrv->shmem_fuzz_len = len;
    ...
    if (fsrv->fs_shm_key) {
      ...
      if (!fs_buffer) memcpy(&fsrv->abs_fp->dfile.stat, &st, sizeof(st));
      else fsrv->abs_fp->dfile.stat.st_size = len;

      fs_buffer = fsrv->abs_fp->dfile.contents;
      ...
      memcpy(fs_buffer, buf, len);

      fsrv->abs_fp->offset = 0;
      fsrv->abs_fp->fd = SHM_AFL_FD;
      fsrv->abs_fp->info.flock_release = 0;
    }
...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
...
#ifdef PROFILING_NYX
    forkserver_spent_time += fsrv->nyx_handlers->nyx_get_reload_time(fsrv->nyx_runner);
#endif
  ...
  u64 fork_time = 0;
  if ((res = read(fsrv->fsrv_st_fd, &fork_time, sizeof(u64))) != sizeof(u64)) {
    if (*stop_soon_p) {return 0;}
    RPFATAL(res, "Unable to request new process from fork server (OOM?)");
  }
  ...
  if (res = read(fsrv->fsrv_st_fd, &prof_time_spent, sizeof(u64)) != sizeof(u64)) {
    if (*stop_soon_p) {return 0;}
    RPFATAL(res, "Unable to request new process from fork server (OOM?)");
  }
  ...
  u64 time_spent = 0;
  if ((res = read(fsrv->fsrv_st_fd, &time_spent, sizeof(u64))) != sizeof(u64)) {
    if (*stop_soon_p) {return 0;}
    RPFATAL(res, "Unable to request new process from fork server (OOM?)");
  }
...
void afl_fsrv_deinit(afl_forkserver_t *fsrv) {
  ...
  afl_fsrv_kill(fsrv);
  shmctl(fsrv->fs_afl_shm_id, IPC_RMID, NULL);
