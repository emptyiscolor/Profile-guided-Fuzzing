...
extern u64 time_spent_running;
extern u64 time_write_testcase;
extern u64 forkserver_spent_time;
extern u64 time_spent_in_user;
extern u64 time_spent_in_sys;
extern u64 prof_total_time;
extern u64 time_post_fuzzing;
extern u64 extra_spending_time;
#endif
...
#ifdef PROFILING_NYX
  plugin->nyx_get_reload_time = dlsym(handle, "nyx_get_aux_reload_time");
  if (plugin->nyx_get_reload_time == NULL) { goto fail; }
#endif
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  if (getenv("FS_AFL_SHM_ID"))     {afl->abs_fs_mode      = 1; afl->fsrv.use_abs_fs = true;}
  ...
  // if (afl->shmem_testcase_mode && afl->abs_fs_mode) {
  //   FATAL(
  //     "abs_fs_mode is conflict with shmem_testcase_mode!"
  //   );
  // }
  ...
  perform_dry_run(afl);
  if (unlikely(getenv("AFL_PERFORM_DRY_RUN_ONLY"))) {
    goto stop_fuzzing;
  }
...
stop_fuzzing:
  ...
  SAYF(cYEL "[!] " cRST
       ...
       time_spent_running / afl->fsrv.total_execs,
       time_write_testcase / 1000000,
       time_write_testcase / afl->fsrv.total_execs,
       forkserver_spent_time / 1000000,
       forkserver_spent_time / afl->fsrv.total_execs,
       prof_total_time / 1000000,
       prof_total_time / afl->fsrv.total_execs,
       time_spent_in_sys / afl->fsrv.total_execs,
       time_spent_in_user / afl->fsrv.total_execs,
       (time_spent_working - time_post_fuzzing + extra_spending_time) / 1000000,
       (time_spent_working - time_post_fuzzing + extra_spending_time) / afl->fsrv.total_execs,
       time_post_fuzzing / 1000000,
       time_post_fuzzing / afl->fsrv.total_execs);
