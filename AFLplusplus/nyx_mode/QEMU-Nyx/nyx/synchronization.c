...
static void perform_reload(void){
	if(fast_reload_root_created(get_fast_reload_snapshot())){
		uint64_t reload_time = 0;
		struct timespec spec;
		uint64_t start_time, end_time;
		clock_gettime(CLOCK_REALTIME, &spec);
		start_time = (spec.tv_sec * 1000000000) + spec.tv_nsec;
		...
		clock_gettime(CLOCK_REALTIME, &spec);
        end_time = (spec.tv_sec * 1000000000) + spec.tv_nsec;
        reload_time = (end_time - start_time);
        set_result_reload_time(GET_GLOBAL_STATE()->auxilary_buffer, reload_time);
	}
	else{
