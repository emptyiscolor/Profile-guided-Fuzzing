...
__attribute__((destructor)) static void dtor()
{
    clock_t end = clock();
    if (begin != 0) {
      fprintf(fp, "\n[+]  PROG[%d] (AFL dtor)  END clock: %Lf\n", getpid(), (long double)(end));
      fprintf(fp, "\n[+]  PROG[%d] (AFL)  Overall time: %Lf\n", getpid(), (long double)(end - begin));
	}
    fclose(fp);
}
#endif
...
static void __afl_start_snapshots(void) {
  ...
  clock_t times_start = times(&tms_tmp);
#endif
  ...
  prof_start_time = (spec.tv_sec * 1000000000) + spec.tv_nsec;
    ...
    clock_t escapted = tmp_clock - times_start;
#endif
...
void __afl_manual_init(void) {
  ...
  if (!init_done) {
    if (getenv("FS_AFL_SHM_ID"))
      dlopen("/tmp/fs_hook.so", RTLD_NOW);

#ifdef PROFILING_RT
    fp = fopen("/tmp/tcpdump_profile.txt", "a");
#endif
    ...
    while (--cnt) {
	     ...
	     // child
             if (pid == 0) {
               begin = clock();
               fprintf(fp, "\n[+]  PROG[%d] (__afl_manual_init)  Begin clock: %Lf\n", getpid(), (long double)(begin));
               return;
	  }
 }
#endif
